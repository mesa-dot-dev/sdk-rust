...
async fn test_get_sync_status() {
    ...
    let name = common::unique_name("sync-status");
    ...
    let resp = repos_api::get_by_org_by_repo_sync(&config, &org, &name)
        .await
        .unwrap();

    // Just verify we got a valid response with a status field.
    let _ = resp.status;
